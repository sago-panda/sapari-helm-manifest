...
| 영역 | 현재 |
|---|---|
| 노드 | 2대 — `kube-master` 192.168.123.105 (control-plane 단일), `kube-worker` 192.168.123.107. 같은 `/24` L2. 커널 6.12, containerd 2.x, k8s 1.35 |
...
> **worker(.107) 먼저** 전환해 검증한 뒤 master(.105)를 전환한다 (control-plane 리스크 최소화).
...
# 해당 노드만 Cilium CNI 활성화 (CiliumNodeConfig per-node override — 공식 문서 양식 사용)
#   → CNI conf 기록 + Calico에서 분리
# 노드의 cilium agent 재시작 후:
kubectl uncordon $NODE

# 검증: 새 Pod가 10.244.x IP를 받고 Running, 노드 간 Pod-to-Pod / Pod-to-Service 통신 OK
```
worker 검증 OK → master(.105) 동일 절차.
